function [phasechange, max_change] = arg_change_calc(f_samp,inv_delta,boxSize,bw)
...
%
  ...
  for ii=-boxSize:(boxSize-1)
    phase_bottom = mod(argc((1+2*boxSize):end,(1+boxSize+ii):(end-boxSize+ii)) ...
        - argc((1+2*boxSize):end,(1+boxSize+ii+1):(end-boxSize+ii+1))+(2*boxSize-bw:N-1-bw)'*ones(1,N-2*boxSize)*pi/inv_delta^2+pi, 2*pi)-pi;
    phase_top = mod(-argc((1):(end-2*boxSize),(1+boxSize+ii):(end-boxSize+ii))-(-bw:N-2*boxSize-1-bw)'*ones(1,N-2*boxSize)*pi/inv_delta^2 ...
        + argc((1):(end-2*boxSize),(1+boxSize+ii+1):(end-boxSize+ii+1))+pi, 2*pi)-pi;
    phase_right = mod(-argc((1+boxSize+ii):(end-boxSize+ii),(1+2*boxSize):end)+ones(N-2*boxSize,1)*(2*boxSize-bw:N-1-bw)*pi/inv_delta^2 ...
    ...
    phase_left = mod(argc((1+boxSize+ii):(end-boxSize+ii),(1):(end-2*boxSize)) ...
        - argc((1+boxSize+ii+1):(end-boxSize+ii+1),(1):(end-2*boxSize))-ones(N-2*boxSize,1)*(-bw:N-2*boxSize-1-bw)*pi/inv_delta^2+pi, 2*pi)-pi;
